...
Scene::Scene(const VulkanContext &vc) : VC(vc) {
    ...
    const auto vert_shader_spv = compiler.CompileGlslToSpv(
        TriangleVertShader,
    ...
    const std::vector<uint> vert_shader_code {
        vert_shader_spv.cbegin(),
        vert_shader_spv.cend()
    };

    const auto vert_shader_module = VC.Device->createShaderModuleUnique({
        {},
        vert_shader_code
    });

    const auto frag_shader_spv = compiler.CompileGlslToSpv(
        TriangleFragShader,
    ...
    const std::vector<uint> frag_shader_code{
        frag_shader_spv.cbegin(),
        frag_shader_spv.cend()
    };

    const auto frag_shader_module = VC.Device->createShaderModuleUnique({
        {},
        frag_shader_code
    });
    ...
    TC.RenderPass = VC.Device->createRenderPassUnique({
        {},
        attachments,
        subpass
    });
    ...
    const vk::PipelineInputAssemblyStateCreateInfo input_assemply{
        {},
    ...
    TC.CommandPool = VC.Device->createCommandPoolUnique({
        vk::CommandPoolCreateFlagBits::eResetCommandBuffer,
        VC.QueueFamily
    });

    TC.CommandBuffers = VC.Device->allocateCommandBuffersUnique({
        TC.CommandPool.get(),
...
bool Scene::Render(uint width, uint height, const vk::ClearColorValue &bg_color) {
    ...
    const auto image_mem_reqs = VC.Device->getImageMemoryRequirements(
        offscreen_image.get()
    );

    uint32_t memory_type = VC.FindMemoryType(
    ...
    const auto offscreen_image_memory = VC.Device->allocateMemoryUnique(alloc_info);

    VC.Device->bindImageMemory(
        offscreen_image.get(),
        offscreen_image_memory.get(),
        0
    );

    // 07_image_views.cpp --- createImageViews
    const auto offscreen_image_view = VC.Device->createImageViewUnique({
        {},
        offscreen_image.get(),
    ...
    TC.ResolveImageMemory = VC.Device->allocateMemoryUnique({
        resolve_image_mem_reqs.size,
        VC.FindMemoryType(
                resolve_image_mem_reqs.memoryTypeBits,
                vk::MemoryPropertyFlagBits::eDeviceLocal
        )
    });
    ...
    VC.Device->bindImageMemory(
        TC.ResolveImage.get(),
        TC.ResolveImageMemory.get(),
        0
    );

    TC.ResolveImageView = VC.Device->createImageViewUnique({
        {},
        TC.ResolveImage.get(),
        vk::ImageViewType::e2D,
        ImageFormat,
    ...
    const std::array image_views {
        *offscreen_image_view,
        *TC.ResolveImageView
    };

    const auto framebuffer = VC.Device->createFramebufferUnique({
        {},
        TC.RenderPass.get(),
        image_views,
        width,
